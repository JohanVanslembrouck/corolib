...
public:
    ...
    async_operation<void> push(TYPE value)
    {
        ...
        {
            print(PRI2, "push(): buffer full: saving value to push: %d\n", value);
        ...
        else
        {
            ...
            if (m_pop_operation_index != -1)
            {
                ...
                print(PRI2, "push(%d): before complete_pop(%d)\n", value, idx);
                complete_pop(idx);
                print(PRI2, "push(%d): after complete_pop(%d)\n", value, idx);
            }
            print(PRI2, "push(%d): ret.completed();\n", value);
            ret.completed();
        }
        return ret;
    }
    ...
    async_operation<TYPE> pop()
    {
        ...
        {
            print(PRI2, "pop(): buffer empty: cannot pop anymore\n");
        ...
        else
        {
            ...
            if (m_push_operation_index != -1)
            {
                ...
                print(PRI2, "pop(): before complete_push(%d)\n", idx);
                complete_push(idx);
                print(PRI2, "pop(): after complete_push(%d)\n", idx);
            }

            print(PRI2, "pop(): ret.set_result_and_complete(%d);\n", readValue);
...
protected:
    ...
    {
        print(PRI1, "complete_push(idx = %d): m_value_to_be_pushed = %d\n", idx, m_value_to_be_pushed);
        ...
        {
            print(PRI2, "complete_push(%d): om_async_operation_t = %p\n", idx, om_async_operation_t);
            print(PRI2, "complete_push(%d): m_writeIndex = %d\n", idx, m_writeIndex);
            print(PRI2, "complete_push(%d): m_value_to_be_pushed = %d\n", idx, m_value_to_be_pushed);
        ...
        {
            print(PRI1, "complete_push(idx = %d): om_async_operation_t = nullptr\n", idx);
        }
    }
    ...
    */
    void complete_pop(int idx)
    {
        ...
        if (om_async_operation_t)
        {
            print(PRI2, "complete_pop(%d): om_async_operation_t = %p\n", idx, om_async_operation_t);
            print(PRI2, "complete_pop(%d): m_readIndex = %d\n", idx, m_readIndex);

            readValue = m_buffer[m_readIndex];
            m_readIndex = (m_readIndex + 1) & (ARRAYSIZE - 1);
            --m_occupiedCells;

            om_async_operation_t->set_result(readValue);
            om_async_operation_t->completed();
        }
        else
        {
            print(PRI1, "complete_pop(idx = %d): om_async_operation_t = nullptr\n", idx);
        }
    }
