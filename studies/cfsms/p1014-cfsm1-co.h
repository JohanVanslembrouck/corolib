/**
 * @file p1014-cfsm1-co.h
 * @brief
 * Contains an implementation of CFSM1 using coroutines and async_task with a taskholder.
 * 
...
public:
    ...
    bool process_message()
    {
        ...
        if (msgid1 == MessageId::Message001_req) {
            int i = m_taskHolder.get_free_index();
            if (i != -1) {
                m_taskHolder.assign(i, std::move(mainflow()));
            }
            else {
                printf("Could not find free entry in taskholder\n");
                return false;
            }
        }
        else
...
private:
    ...
    const static int NR_TASKS = 4;
    TaskHolder<NR_TASKS, void> m_taskHolder;
};
