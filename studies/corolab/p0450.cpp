 ...
 *
 *        Based upoon p0440.cpp, but uses a modified version
 *        of the future header file, called p0450_future, that
 *        minimizes the number of threads.
 *        However, several (dirty) modifications were necessary
 *        to the original implementation of future and promise, 
 *        probably breaking their original behavior.
 *        
...
//#include <future>
#include "p0450_future"

//--------------------------------------------------------------

future<int> function5(int a, int b)
{
#if 1
    print("function5(%d, %d) 1: auto f6 = std::async([=]() {...});\n", a, b);
    future<int> f6 = std::async([=]() {
        print("std::async(...) 1: int c = %d + %d;\n", a, b);
        int c = a + b;
        print("std::async(...) 2: std::this_thread::sleep_for(std::chrono::milliseconds(3000));\n");
        std::this_thread::sleep_for(std::chrono::milliseconds(3000));
        print("std::async(...) 3: return c = %d;\n", c);
        return c;
        });

    print("function5(%d, %d) 2: int c = co_await f6; : &f6 = %p\n", a, b, &f6);
    int c = co_await f6;
#else
    print("function5(%d, %d) 1: int c = %d + %d;\n", a, b);
    int c = a + b;
#endif
    print("function5(%d, %d) 3: int d = a + c + 5;\n", a, b);
    int d = a + c + 5;
    print("function5(%d, %d) 4: co_return %d;\n", a, b, d);
    co_return d;
}

future<int> coroutine4(int a, int b)
{
    print("coroutine4(%d, %d) 1: future<int> f5 = function5(a, b);\n", a, b);
    future<int> f5 = function5(a, b);
...
int main()
{
    ...
    f1.wait();
    print("main() 3: std::this_thread::sleep_for(std::chrono::milliseconds(1000));\n");
    std::this_thread::sleep_for(std::chrono::milliseconds(1000));
    print("main() 4: return 0;\n");
    return 0;
}
