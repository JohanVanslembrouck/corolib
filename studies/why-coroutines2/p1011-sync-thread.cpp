...
void synchronous(int st) {
    ...
    print(PRI1, "synchronous: %d: read complete: write 2\n", i);       // second write after the read
    i = write(i + 1);
    assert(i == st + 5);

    print(PRI1, "synchronous: %d: write 2 complete: close\n", i);
    i = close(i + 1);
    assert(i == st + 6);
...
int main() {
    ...
    std::jthread thr1{ synchronous, 10 };
    std::jthread thr2{ synchronous, 20 };
    print(PRI1, "main: end\n");
    return 0;
}
