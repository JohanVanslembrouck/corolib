...
void asynchronous_start_op(int st) {
    ...
    print(PRI1, "asynchronous_start_op: %d: read complete: write 2\n", i);      // Second write after read
    auto st_write2 = start_write(i + 1);
    i = st_write2.get_result();
    assert(i == st + 5);

    print(PRI1, "asynchronous_start_op: %d: write 2 complete: close\n", i);
...
void asynchronous_operations(int st) {
    int i = st;
    print(PRI1, "asynchronous_operations: begin\n");
    ...
    print(PRI1, "asynchronous_operations: %d: read complete: write 2\n", i);        // Second write after read
    ops.start_write(i + 1);
    i = ops.get_result();
    assert(i == st + 5);

    print(PRI1, "asynchronous_operations: %d: write 2 complete: close\n", i);
    ops.start_close(i + 1);
    i = ops.get_result();
    assert(i == st + 6);
...
int main() {
    ...
    std::jthread thr3([] { runEventQueue(evqueuethr, 2 * 7); });
    thr3.detach();
    thr1.join();
    thr2.join();
    print(PRI1, "main: end\n");
