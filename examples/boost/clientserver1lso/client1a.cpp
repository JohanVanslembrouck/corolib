...
public:
    ...
    {
        // Connecting
        print(PRI1, "mainflow: connect_operation sc = start_connecting();\n");
        connect_operation sc = start_connecting(m_ep);
        ...
        // Writing
        print(PRI1, "mainflow: write_operation sw = start_writing(...);\n");
        write_operation sw = start_writing(str1.c_str(), str1.length() + 1);
        print(PRI1, "mainflow: co_await sw;\n");
        co_await sw;

        // Reading
        print(PRI1, "mainflow: read_operation sr = start_reading();\n");
        read_operation sr = start_reading();
        ...
        print(PRI1, "mainflow: timing_operation st = start_timer(100);\n");
        timing_operation st = start_timer(client_timer, 100);
    ...
    task<int> mainflow()
    {
        ...
        for (int i = 0; i < 100; i++)
        {
            ...
            print(PRI1, "mainflow: task<int> task = mainflow(i, counter);\n");
