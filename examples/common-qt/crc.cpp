...
const unsigned long crc_32_table[] = { /* CRC polynomial 0xedb88320 */
...
unsigned char bufferOut[4] = {1, 1, 1, 1};
 ...
 * @return
 */
unsigned long CalculateCRC32(unsigned char* buffer, unsigned long buffersize)
{
    unsigned long crc32 = CRC32_SEED;
    /* pre seed the calculation */
    if (buffer != NULL)
    {
        unsigned char* buf = buffer;
        unsigned long size = buffersize;
        /* sanity check for null buffer pointer */
        while (size-- > 0)
        {
            crc32 = crc_32_table[((crc32) ^ (*buf++)) & 0xff] ^ ((crc32) >> 8);
        }
    }
    crc32 ^= CRC32_SEED;
    return crc32;
