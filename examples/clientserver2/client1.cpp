...
public:
	...
	async_task<int> performAction(int timeout)
	{
		...
		case 0:	// Reply has arrived, stop the timer and print the result
		{
			...
			print(PRI1, "performAction: async_task<int> ca = acknowledgeAction();\n");
			async_task<int> ca = acknowledgeAction();
			print(PRI1, "performAction: co_await ca;\n");
			co_await ca;
		}
		break;
		case 1: // Timer has expired, send a cancel request
		{
			...
			// Canceling
			print(PRI1, "performAction: async_task<int> ca = cancelAction();\n");
			async_task<int> ca = cancelAction();
			print(PRI1, "performAction: co_await ca;\n");
			co_await ca;
